...
- **拳** トグル … 押すたびに 通常 → **石の拳のみ** → **石の拳を除外** と回る。
  石の拳（Martial Artist の「石拳の道」）はそのキャラ専用なので、丸ごと外せる。
  外すと専用 mod が出ず、変化後の文にも検索が当たらない（⚙ で既定にもできる）
- **培** トグル … 培養のオーブで置換できる行に ◆、Vaal ユニークにバッジ
- **スキル**タブ … スキルをタグ（アタック / 近接 / ノヴァ / スラム …）の AND で絞る専用ページ
- ジェムの**装備条件**（片手メイス / 両手メイス …）も検索対象。「メイス」で引ける。
  検索語が装備条件に当たったときは、一覧にもその行を出す（なぜ出たのかが分かるように）
- 日本語 / EN / 両方 を切り替え。訳が無い行は `EN` 印を付けて英語で出す
- **検索部は一覧と同じ幅の帯**（色・境界線・影・下側の角丸）で、一覧と地続きに見えないようにする
...
hw:off                   石の拳を検索から外す（専用 mod も変化後の文も当てない）
