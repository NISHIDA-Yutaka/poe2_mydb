...
- 装飾（角丸・影・グラデーション）は情報の区別に使うときだけ入れる
- 動きは**場所を作るためだけ**に使う。検索部の開閉のような「読む面積を広げる」動きは入れるが、
  結果の中身が勝手に動く・消えることはしない。開閉しても**状態は必ず要約で見える**（§8.7）
...
- **暗（既定）と明の 2 テーマ**。切り替えは CSS 変数だけで行い、個別の色を直書きしない。
  選択は `localStorage` と URL（`theme=light`）に保存する。
  どちらのテーマでも本文のコントラストを確保する（§8.1 の色の下限を明側でも満たす）。
  **アイテム画像の下地だけは明テーマでも暗いままにする** — ゲーム内の暗い背景を前提に
  描かれた画像なので、白地に置くと輪郭が飛ぶ
- フォントはシステム標準
...
ヘッダ（検索欄とチップ群）は縦に場所を取るので、**普段は畳んで結果に面積を譲る**。

- 開く条件は **ホバー / 入力中（`:focus-within`）/ 📌 で固定**の 3 つ。
  入力中に開いたままになるのが重要で、マウスが離れても打っている間は閉じない
- 畳んでいる間は**今かかっている絞り込みを要約行に出す**（`summaryHTML()`）。
  状態が見えないまま隠すことはしない（§8.1 の「情報を隠さない」）
- 畳んでいるときは検索欄自体も小さくし、開くと元の大きさに戻す
- 実装は CSS の `:hover` / `:focus-within` / `.pin` で `#filters` の `max-height` を切り替えるだけ。
  JS は 📌 の状態（`localStorage`）のみ持つ

### 8.8 アイコン

一覧のアイコンは既定 **64px**（スキル表は 38px）。装備の見た目で判別できることを優先し、
小さくして行数を稼ぐことはしない。下地は暗色 + 枠線で、画像が無いときは枠ごと隠す。
大きさは §8.9 の設定ウィンドウで変えられる。

### 8.9 表示設定ウィンドウ（⚙）

**見やすさの最終判断はユーザーに委ねる。** 既定値はこちらで決めるが、環境（画面サイズ・
解像度・視力）で最適は変わるので、効いてくる値は設定で変えられるようにする。

| 項目 | 変数 | 既定 | 範囲 |
|---|---|---|---|
| アイコン | `--ico` | 64px | 32–160px |
| スキル表のアイコン | `--ico-sm` | 38px | 20–96px |
| 文字サイズ | `--fs` | 14px | 11–22px |
| 表示幅 | `--app-w` | 1180px | 900–2400px |
| 主要色 | `--accent`（`--unique` も追従） | 暗 `#c8a45c` / 明 `#8a6410` | 自由 |
| 操作色 | `--accent2` | 暗 `#7aa2f7` / 明 `#2350a8` | 自由 |

実装のきまり:

- **CSS の固定 px は禁止**。本文サイズは `--fs` から `calc()` で派生させる
  （`--fs-lg` / `--fs-s` / `--fs-xs` …）。新しい要素を足すときもこの変数を使う
- 色は**テーマごとに保存**する（暗で選んだ金色を明テーマに持ち込まない）
- アクセント上の文字色 `--on-fg` は**背景の相対輝度から自動で決める**（`readableOn()`）。
  ユーザーがどんな色を選んでもチップの文字が読めなくならないため、ここは設定させない
- 変更は入力のたびに即反映し、`localStorage`（`poe2db.ui`）に保存する
- 「既定に戻す」で全項目を初期値に戻せる
- 上部に見本（実データのアイコン + 名前 + 効果 1 行）を出し、閉じなくても結果を確認できる
